...
// DONE: Chain together a `map` and a `reduce` call to get a rough count of all words in all articles.
  Article.numWordsAll = () => {
    return Article.all.map(function(allArticles){
      return allArticles.body.split(' ').length;
    }).reduce(function(accummulative, current) {
      return accummulative + current
    })
  };
...
// DONE: Chain together a `map` and a `reduce` call to produce an array of unique author names.
  Article.allAuthors = () => {
    return Article.all
    .map(uniqueAuthor => uniqueAuthor.author)
    .reduce((array, newAuthorArray) => {
      array.indexOf(newAuthorArray) < 0 ?
        array.push(newAuthorArray): null;
      return array;
    },[]);
  };
