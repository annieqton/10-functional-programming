...
(function(module) {
  ...
  Article.allAuthors = () => {
    return Article.all.map(function(uniqueAuthors){
      return uniqueAuthors.author;
    }).reduce(function(previous,current){
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(author => {
      return {
        // DONE: Complete the value for this object property
        name: author,

        // DONE: Complete these three FP methods.
        numWords: Article.all.filter(function(articleBody){
          if(articleBody.author === author) {
            return articleBody.body;
          }
        }).map(function(articleBody) {
          return articleBody.body.split(' ').length;
        }).reduce(function(previous, current) {
          return previous + current
        })
      }
